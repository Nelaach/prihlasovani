...
class Auth extends CI_Controller {
    ...
    public function save() {
        if ($this->ion_auth->logged_in()) {
            ...
            $data['knihy'] = $this->db->query('SELECT * FROM knihy')->result();
            $this->load->model('Cetba_model');
            $data['polozky'] = $this->Cetba_model->get_menu();


            $n = $this->input->post('nazev_knihy');
            $e = $this->input->post('autora');
            $p = $this->input->post('kategorie_idkategorie');




            $kurz = $this->db->query("select * from knihy where nazev_knihy=?", [$n]);
            $omezeni = $kurz->num_rows();

            if ($omezeni >= 1) {
                ?>
                <h3 class="centered center" color="red"> Tento název již existuje</h3>
                <style>
                    .centered {
                        color: red;
                        position: fixed;
                ...
                $this->db->query("insert into knihy (nazev_knihy, autora, kategorie_idkategorie) values(?, ?, ?)", [$n, $e, $p]);
                ?>
                <h3 class="centered center" color="red"> Úspěšně přidáno</h3>
                <style>
                    .centered {
                        color: red;
                        position: fixed;
